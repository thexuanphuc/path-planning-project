...
class AnimatedCustomRRTStar(AnimatedRRTStar):
    """Custom RRT* with clearance cost"""
    def __init__(self, name, start, goal, obstacles, bounds, color='orange', clearance_weight=1.0, **kwargs):
        super().__init__(name, start, goal, obstacles, bounds, color, **kwargs)
    ...
    def get_clearance_cost(self, state):
        ...
        for obs in self.obstacles:
            if hasattr(obs, 'radius'):
                ...
                d = np.linalg.norm(obs.center - state)
            if d < min_dist:
                min_dist = d
        if min_dist <= 0:
            return 100.0
    ...
    def choose_parent(self, new_node, near_inds):
        if not near_inds:
            return None
        costs, possible_parents = [], []
        node_clearance = self.get_clearance_cost(new_node.state)
        for i in near_inds:
            near_node = self.node_list[i]
            t_node = self.steer(near_node, new_node)
            if t_node and is_collision_free(near_node.state, t_node.state, self.obstacles):
                costs.append(near_node.cost + get_dist(near_node.state, t_node.state) + node_clearance)
                possible_parents.append(near_node)
        if not costs:
            return None
        min_ind = costs.index(min(costs))
...
def create_all_planners_animation():
    # Environment
    ...
    print("Running all 4 different planners and recording snapshots...")
    
    # Create all 4 different planners
    planners = [
        AnimatedRRTStar("RRT*", start, goal, obstacles, bounds, color='red', 
                       step_size=0.8, max_iter=200, search_radius=3.0),
        AnimatedRRTStar("Informed RRT*", start, goal, obstacles, bounds, color='green', 
                       step_size=0.8, max_iter=200, search_radius=3.0),
        AnimatedCustomRRTStar("Custom RRT*", start, goal, obstacles, bounds, color='orange',
                             clearance_weight=1.0, step_size=0.8, max_iter=200, search_radius=3.0),
        AnimatedRRTStar("BIT*-style", start, goal, obstacles, bounds, color='blue', 
                       step_size=0.6, max_iter=200, search_radius=2.5),
    ]
    ...
    def update(frame):
        ...
        fig.suptitle(f'4 Different Planners Comparison - Frame {frame+1}/{max_snapshots}', 
    ...
    anim.save('all_planners_comparison.gif', writer=writer, dpi=100)
    print("Animation saved to all_planners_comparison.gif")
    plt.close()

if __name__ == "__main__":
    create_all_planners_animation()
